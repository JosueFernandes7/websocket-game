...
class ClientGUI:
    ...
    def create_widgets(self):
        ...
        self.text_area = scrolledtext.ScrolledText(
            self.master, wrap=tk.WORD, state="disabled", width=60, height=20
        )
        ...
        self.send_button = tk.Button(
            self.master, text="Enviar", command=self.send_input
        )
        self.send_button.grid(row=1, column=1, padx=5, pady=5)

        # Botão de jogar novamente
        self.restart_button = tk.Button(
            self.master, text="Jogar Novamente", command=self.restart_game
        )
        ...
        try:
            while self.connected:
                ...
                if not server_message:
                    ...
                    self.append_message("O servidor encerrou a conexão.")
                    break
            ...
            # Captura erros de desconexão
            self.append_message("Conexão perdida com o servidor.")
        finally:
            self.connected = False
            self.restart_button.config(state="normal")
            self.input_field.config(state="disabled")
